...
public record CameraProfile(String id, double shakeIntensity, double shakeMoveMultiplier, double handheldShakeIntensity,
                            int targetFov,
                            @IntRange(from = 0, to = 59) int pathStabilization) implements ConfigurationSerializable {
    ...
    public static CameraProfile deserialize(@NotNull Map<String, Object> args) {
        return new CameraProfile(
                (String) args.get("id"),
                (double) args.get("shakeIntensity"),
                (double) args.get("shakeMoveMultiplier"),
                (double) args.get("handheldShakeIntensity"),
                (int) args.get("targetFov"),
                (int) args.get("pathStabilization")
        );
    }
